...
I send the same email, often. The idea is that this script just reads from a file of different txt files. It then just input the text when you chose which file you want to input.
...
This serves a few purposes, but a nice side effect is that it means I can have information that I'd want to be confidential in the body of emails, etc.

Another nice thing is formatting can be added because we're just sending input, of course. Here's a nice example:

#+begin_src txt

Hi there,

I'm on the case with this task{!} !{i}All under control{!}!{i}

Best regards, ^+{Up 5}!hffSegoe UI Light

#+end_src

The =^+{Up 5}!hffSegoe UI Light= selects all the text (~Ctrl+Up, Up, Up, ...~), and then ~Alt, h, ff~ to get to the font field in Outlook 2007, and finally we type our desired font!

Obviously, this is only useful for certain clients, but it's trivial to configure them for what you need, as is the case with most AutoHotkey.
