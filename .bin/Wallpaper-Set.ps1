##   Wallpaper-Set.ps1  ---  Set the wallpaper and add text to it.

# Copyright (c) 2022   Julian Orchard <jorchard@pm.me>

## Description:

# This wallpaper setter uses ImageMagick to add text with the current time
...
# COLOURS
  ...
  # Get Current and Saved Colours
    $currentColor = Get-Content -Path "C:\cmd\bin\vars\colours.txt" -TotalCount 1
    $savedColor   = Get-Content -Path "C:\cmd\bin\vars\colours-saved.txt" -TotalCount 1
  # Copy Current Colour to Saved Colour
    Copy-Item "C:\cmd\bin\vars\colours.txt" -Destination "C:\cmd\bin\vars\colours-saved.txt"
  }
  catch 
  { 
    $currentColor = "#FFF" 
